...
begin
lemma domain:"\<exists>!A. (\<forall>x. (\<exists>y. (x, y) \<in> G) \<longleftrightarrow> x \<in> A)" by(auto)
lemma range:"\<exists>!B. (\<forall>y. (\<exists>x. (x, y) \<in> G) \<longleftrightarrow> y \<in> B)" by(auto)
lemma sub_proj_prod:"G \<subseteq> (fst`G) \<times> (snd`G)"  by (simp add: subset_fst_snd)
end


section Correspondence
locale correspondence=graph G for G::"('a \<times> 'b) set"+fixes A::"'a set" and B::"'b set" assumes domain_sub_source:"fst`G \<subseteq> A" and codomain_sub_target:"snd`G \<subseteq> B"
begin
lemma pair_elemE1:"(x, y) \<in> G \<Longrightarrow> x \<in> fst`G" by (simp add: Domain.DomainI fst_eq_Domain)
lemma pair_elemE2:"(x, y) \<in> G \<Longrightarrow> y \<in> snd`G" by (simp add: Range.intros snd_eq_Range)
lemma domain_sub_source2:"G``X \<subseteq> snd`G" using sub_proj_prod by blast      
lemma image_domain_eq_codomain:"G``(fst`G) = snd`G" by(auto intro: pair_elemE2 pair_elemE1)
lemma image_iso:"X \<subseteq> Y \<Longrightarrow> G``X \<subseteq> G``Y"  by (simp add: Image_mono)
lemma empty_image:"G``{} = {}" by simp
lemma ne_image:"X \<subseteq> fst`G \<Longrightarrow> X \<noteq> {} \<Longrightarrow> G``X \<noteq> {}"  by fastforce
lemma image_iso_corr:"fst`G \<subseteq> A \<Longrightarrow> G``A = snd`G"  by (metis domain_sub_source2 image_domain_eq_codomain image_iso subset_antisym)
lemma im_singleton:"y \<in> G``({x}) \<longleftrightarrow> (x,y) \<in> G" by auto
end


locale inj_corr=correspondence+ assumes inj:"(x1, y) \<in> G \<Longrightarrow> (x2, y) \<in> G \<Longrightarrow> x1 = x2"
locale sur_corr=correspondence+assumes sur:"G``A = B" begin lemma sur_exE: "y \<in> B \<Longrightarrow> (\<exists>x \<in> A. (x, y) \<in> G)" using sur by blast end
locale coinj_corr=correspondence+assumes coinj:"(x, y1) \<in> G \<Longrightarrow> (x, y2) \<in> G \<Longrightarrow> y1=y2"
begin
lemma unique_im: "x \<in> A \<Longrightarrow> G``{x} \<noteq> {} \<Longrightarrow> (\<exists>!y. y \<in> G``{x})"using coinj by auto
lemma unique_im2: "x \<in> A \<Longrightarrow> G``{x} \<noteq> {} \<Longrightarrow> is_singleton (G``{x})"  apply(erule is_singletonI') by (simp add:coinj)
lemma unique_im3: "x \<in> A \<Longrightarrow> G``{x} \<noteq> {} \<Longrightarrow> (\<exists>!y. (x, y) \<in> G)" using unique_im by auto
definition func where "func \<equiv> (\<lambda>x. if x \<in> A \<and> G``{x} \<noteq> {} then (the_elem (G``{x})) else undefined)"
lemma fsimp0: "x \<in> A \<Longrightarrow> G``{x} \<noteq> {} \<Longrightarrow> (x, func x) \<in> G" by (metis (no_types) func_def im_singleton is_singleton_the_elem singletonI unique_im2)
end

locale cosur_corr=correspondence+assumes cosur:"(converse G)``B = A"
begin
lemma cosurE: "x \<in> A \<Longrightarrow> G``{x} \<noteq> {}"  using cosur by blast
lemma cosurE2: "x \<in> A  \<Longrightarrow> (\<exists>y. y \<in> G``{x})" using cosurE by blast
end

section FunctionalCorrespondence
locale fun_corr=coinj_corr+cosur_corr
begin
lemma fsimp: "func = (\<lambda>x. if x \<in> A then (the_elem (G``{x})) else undefined)" unfolding func_def using cosurE by meson
lemma fsimp2: "x \<in> A \<Longrightarrow> (x, func x) \<in> G" by (simp add: cosurE fsimp0)
lemma fsimp3:  "x \<in> A \<Longrightarrow> (x, y) \<in> G \<Longrightarrow> y = func x"  by (simp add: coinj fsimp2)
lemma fsimp4:  "x \<in> A \<Longrightarrow> (THE y. (x, y) \<in> G) =  func x"using coinj fsimp2 by blast
lemma fsimp5:  "x \<in> A \<Longrightarrow>  func x \<in> B"  by (meson codomain_sub_target fsimp2 pair_elemE2 subset_iff)
lemma fsimp6:  "x \<notin> A \<Longrightarrow> func x = undefined"by (simp add: fsimp)
lemma func_in_hom:"func \<in> hom A B"  by (simp add: fsimp5 fsimp6 hom_memI)
end
...
locale inj_func=fun_corr+inj_corr
begin lemma injE:"\<And>x y. x \<in> A \<Longrightarrow> y \<in> A \<Longrightarrow> func x = func y \<Longrightarrow> x = y"  by (metis fsimp2 inj) lemma inj_on_alt:"inj_on func A"   by (simp add: injE inj_onI) end
locale bij_func=fun_corr+inj_corr+sur_corr
lemma gr_sub:assumes "graph G1" and "graph G2" shows "G1 \<subseteq> G2 \<longleftrightarrow> (\<forall>x. G1``({x}) \<subseteq> G2``({x}))" by auto
...
locale homomorphism2 =
  dom:magma X "(\<cdot>)" + codom:magma Y"(\<star>)" + map:fun_corr G X Y
   for X::"'a set" and domain_law (infixl "\<cdot>" 70) and Y::"'b set" and codomain_law (infixl "\<star>" 70) and G+
  assumes commutes_with_composition: "\<lbrakk>x \<in> X; y \<in> X \<rbrakk> \<Longrightarrow> (map.func (x \<cdot> y)) = map.func x \<star> map.func y" and
          into:"\<And>x. x \<in> X \<Longrightarrow> (map.func) x \<in> Y"
begin

lemma hom:
  "homomorphism X (\<cdot>) Y (\<star>) map.func"
  by (simp add: codom.magma_axioms commutes_with_composition dom.magma_axioms homomorphism_axioms_def homomorphism_def into)

end

sublocale homomorphism2 \<subseteq> hom:homomorphism X "(\<cdot>)" Y "(\<star>)" "map.func"
  by (simp add: hom)
