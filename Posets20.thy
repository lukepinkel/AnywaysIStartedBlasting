...
lemma fpow_neD0:"A \<in> Fpow_ne X \<Longrightarrow> A \<in> Pow X " 
  by (simp add: fpow_ne_iff2)

lemma fpow_neD0b:"A \<in> Fpow_ne X \<Longrightarrow> A \<in> Pow_ne X" 
...
lemma fpow_ne_iso1:"A \<in> Fpow_ne X \<Longrightarrow> B \<in> Fpow_ne A \<Longrightarrow> B \<in> Fpow_ne X"
...
lemma inf_le5:
  ...
  by (meson inf_le1 binf_commute inf_le3 is_infE1)
...
lemma is_dwdirI1:
  assumes "\<And>A. \<lbrakk>A \<subseteq> X; finite A; A \<noteq> {}\<rbrakk> \<Longrightarrow>  (\<exists>c \<in> X. c lb A)"
  shows "is_dir X (\<ge>)"
proof(rule is_dirI1)
  show "\<And>a b. a \<in> X \<Longrightarrow> b \<in> X \<Longrightarrow> \<exists>c\<in>X. a \<ge> c \<and> b \<ge> c"
  proof-
    fix a b assume "a \<in> X" and "b \<in> X"
    then obtain "{a, b} \<subseteq> X" and "finite {a, b}" and "{a, b} \<noteq> {}"   by blast
    then obtain c where "c \<in> X" and "c lb {a, b}" using assms[of "{a,b}"] by auto
    then obtain "c \<in> X" and "a \<ge> c" and "b \<ge> c" using lb_doubleE1 lb_doubleE2 by blast
    then show "\<exists>c\<in>X. a \<ge> c \<and> b \<ge> c" by auto
  qed
qed
...
proof(rule dir_finite1)
  show "\<And>(a1::'a) a2::'a. a1 \<in> X \<Longrightarrow> a2 \<in> X \<Longrightarrow> a1 \<in> X" by simp
  show "\<And>(a1::'a) a2::'a. a1 \<in> X \<Longrightarrow> a2 \<in> X \<Longrightarrow> a2 \<in> X" by simp
  show "transp_on X (\<ge>)" by simp
qed


lemma dwdir_finite2:
  assumes dir:"is_dir (X::'a::order set) (\<ge>)" and
          sub:"A \<subseteq> X" and
          fin:"finite A" and
          nem:"A \<noteq> {}"
  shows "(\<exists>c \<in> X. c lb A)"
proof-
  obtain "(\<And>a b. \<lbrakk>a \<in> X; b \<in> X\<rbrakk> \<Longrightarrow> (\<exists>c \<in> X.  a \<ge> c \<and>  b \<ge> c))"  using dir is_dirE1[of X "(\<ge>)"] by blast
  then obtain c where "c \<in> X" and "\<forall>a. a \<in> A \<longrightarrow> a \<ge> c" using dir sub fin nem dwdir_finite1[of X A] by blast
  then show "(\<exists>c \<in> X. c lb A)"  using lbI by blast
qed
...
lemma is_dwdirI5:
  assumes isl:"is_inf_semilattice X" and sub:"A \<subseteq> X" and ffc:"is_finf_closed X A" 
  shows "is_dir (A) (\<ge>)"
proof(rule is_dwdirI2) 
  show "\<And>a b. a \<in> A \<Longrightarrow> b \<in> A \<Longrightarrow> \<exists>i. is_inf A {a, b} i"
  proof-
    fix a b assume a0:"a \<in> A" and a1:"b \<in> A" 
    from isl obtain i where i0:"is_inf X {a, b} i"  using a0 a1 sinfD3 sub by blast 
    from ffc i0 a0 a1 inf_equality is_finf_closedE1 obtain i1:"i \<in> A"  by blast 
    then obtain "is_inf A {a, b} i" using inf_in_subset[of "{a, b}" A X i]  using a0 a1 i0 sub by blast
    then show "\<exists>i. is_inf A {a, b} i" by blast
  qed
qed
...
lemma filter_closure_memI3:
  "\<lbrakk>A = {};is_greatest X top\<rbrakk> \<Longrightarrow> top \<in> filter_closure X A"
...
lemma filter_closure_eq_cl:
  assumes isl:"is_inf_semilattice X" and sub:"A \<subseteq> X" and top:"is_greatest X top"
  shows "filter_closure X A = (cl_from_clr (filters_on X)) A "
proof(cases "A={}")
  case True
  then have "is_least (ubd (filters_on X) {A}) (filter_closure X A)"  using filter_closure_ne_lub top by auto
   then show ?thesis  by (simp add: cl_from_clr_def least_equality2) 
next
  case False 
  then have "is_least (ubd (filters_on X) {A}) (filter_closure X A)"  by (simp add: filter_closure_lub isl sub)
  then show ?thesis by (simp add: cl_from_clr_def least_equality2) 
qed
...
lemma filters_on_lattice_inf01:
  ...
  by (metis IntE bsup_idem1 in_mono is_filterE1)
...
lemma filter_on_lattice_top:
  ...
  by (meson is_filter_def filter_on_lattice_top0 greatest_iff insert_subset)

lemma filter_on_lattice_eq:
  assumes lat:"is_lattice X" and fil1:"is_filter X F1" and fil2:"is_filter X F2"
  shows "(F1 \<inter> F2) = {y. (\<exists>f1 \<in> F1. \<exists>f2 \<in> F2. y = Sup X {f1, f2})}"
proof(auto simp add:set_eq_iff)  
  show "\<And>x. x \<in> F1 \<Longrightarrow> x \<in> F2 \<Longrightarrow> \<exists>f1\<in>F1. \<exists>f2\<in>F2. x = Sup X {f1, f2}"  by (metis bsup_idem1 fil1 in_mono is_filterE1)
  show "\<And>f1 f2. f1 \<in> F1 \<Longrightarrow> f2 \<in> F2 \<Longrightarrow> Sup X {f1, f2} \<in> F1"  using fil1 fil2 filters_on_lattice_inf02 lat by blast
  show "\<And>f1 f2. f1 \<in> F1 \<Longrightarrow> f2 \<in> F2 \<Longrightarrow> Sup X {f1, f2} \<in> F2"  by (metis doubleton_eq_iff fil1 fil2 filters_on_lattice_inf02 lat) 
qed
...
lemma filters_on_lattice_sup1b:
  ...
  by (metis Posets20.is_filter_def cSup_least empty_Union_conv filter_closure_filter finite.simps finite_UnionD finite_has_minimal lattD41)
...
proof-
  ...
  by (metis A1 A2 A3 A4 Pow_empty filter_closure_memE1 insertCI is_filterE1 subset_Pow_Union subset_singletonD)
  ...
  have B4:"Fa \<union> Fb \<subseteq> X" 
      by (meson A1 B3 Sup_le_iff dual_order.trans is_filterE1)
  have B5:"Inf X (Fa \<union> Fb) \<in>  (filter_closure X (\<Union>EF))"  
    by (metis A0 B3 B4 binf_finite2 dual_order.refl filter_closure_memI1 is_infE1 lattD41 ne_subset_ne)
...
lemma filters_on_inf_lattice_sup2b:
  ...
  by (metis Sup_least filter_closure_empty filter_closure_filter is_filterE1 min_filter1)
...
lemma filters_on_inf_lattice_sup2c:
  ...
  by (metis Sup_least all_not_in_conv empty_Union_conv filter_closure_filter is_filterE1)
...
lemma filters_on_lattice_sup2b:
  ...
  using filters_on_inf_lattice_sup2c lattD41 by blast
...
lemma filters_on_lattice_sup3b:
  ...
  by (metis Sup_le_iff all_not_in_conv empty_Union_conv filter_closure_filter is_filterE1 lattD41)
...
lemma filters_on_lattice_bsup1:
  ...
  by (simp add: filter_closure_memE1 filters_on_lattice_bsup0 is_filterE1)
...
lemma filters_on_top_inf_semilattice_is_cinf:
  ...
  by (metis clatD2 clr_is_clattice empty_iff filter_is_clr greatest_exD1 pow_is_clattice)
...
lemma lattice_filter_dunion2:
  ...
  by (smt (z3) Posets20.is_filter_def Union_iff filters_on_iff is_ord_cl_def subset_iff)
...
lemma lattice_filter_dunion5:
  ...
  by (meson filter_finf_closed_isl filters_on_iff lattD41 lattice_filter_dunion4 subset_iff)
...
proof-
  ...
  proof
    ...
    by (simp add: A0 A1 B2 filter_finf_closed_isl lattD41)
    have B30:"Inf X {y1, y2} \<in> X \<and> a \<in> X \<and> x1 \<in> X \<and> x2 \<in> X \<and> y1 \<in> X \<and> y2 \<in> X"
    using A1 A2 A3 B2 B3 G_def is_filterE1 by auto
...
lemma primefilterI2:
  assumes A0:"is_lattice X" and A1:"is_pfilter X F" and 
...
lemma prime_filter_compl8: "\<lbrakk>is_lattice X; is_pfilter X F;  sup_prime X F\<rbrakk> \<Longrightarrow> is_ideal X (X-F)"  by (meson Diff_subset is_filter_def filter_compl1 idealI1 is_ord_cl_comp1 is_pfilterD1 prime_filter_compl6)
...
lemma is_dwdir_obtain:
  ...
  using assms is_dwdirE1[of F A B] by auto
...
proof-
  ...
  then show "A \<inter> B \<in> F" using \<open>(C::'a::type set) \<in> (F::'a::type set set)\<close> calculation is_ord_clE1 upc by blast 
qed
...
proof(rule filterI1)
 ...
 show "is_ord_cl (Pow X) F (\<subseteq>)" by(auto intro:upc is_ord_clI1)
qed
...
lemma sets_filter:
  ...
  shows sets_filter_nem:"F \<noteq> {}" and
        ...
        using assms filterD1 apply blast
        apply (simp add: assms filterD2)
        apply (meson assms filterD2 filterD3 filterD4 is_dwdirD1sets)
        using assms filter_memI apply(blast)
        by (meson Pow_iff Pow_top assms filter_greatestD2 greatest_iff)
...
proof(rule is_pfilterI1) 
  ...
  show "Pow X \<noteq> F" using A2 by simp
qed
...
lemma sets_pfilter:
  ...
  shows sets_pfilter_nem:"F \<noteq> {}" and 
        ...
        using assms filterD1 is_pfilter_def apply blast
        apply (simp add: assms is_pfilterD3)
        using assms is_pfilter_def apply blast
        using assms sets_filter_dir is_pfilterD1 apply blast
        using assms is_pfilterD1 sets_filter_upc apply blast
        by (meson Pow_bottom assms empty_subsetI is_pfilterD4 least_iff)
...
lemma sets_pfilter2:
  ...
  shows   sets_pfilter2_dir:"is_dir F (\<lambda>(x::'a set) y::'a set. y \<subseteq> x)" and
          ...
          using assms filterD3 is_pfilterD1 apply blast
          by (simp add: assms filterD4 is_pfilterD1)
...
proof(rule ccontr)
  ...
  then have "(X-A) \<inter> A \<in> F" using A0 A1 sets_filter_dir by blast  
...
lemma pfilters_sets_comp4:
   "\<lbrakk>pfilter (Pow X) F; A \<subseteq> X;  A \<notin> F\<rbrakk> \<Longrightarrow> \<forall>U \<in> F. U - A \<noteq> {}"
  by (meson Diff_eq_empty_iff Pow_iff filter_memI)
...
proof-
 ...
 proof(rule is_dwdirI1)
    ...
    qed
 qed
 also show "is_ord_cl (Pow X) H (\<subseteq>)"  apply(rule is_ord_clI1) using H_def inf.absorb_iff2 by fastforce
 then show "is_filter (Pow X) H"
 by (simp add: \<open>(H::'a::type set set) \<noteq> {}\<close> \<open>(H::'a::type set set) \<subseteq> Pow (X::'a::type set)\<close> calculation filterI1)
qed  
...
lemma finer_proper_filter:
  ...
  using A0 H_def is_pfilterD3 by fastforce
...
lemma set_filter_inter_ne:
  ...
  shows "X \<in> \<Inter>EF"
proof-
  have B0:"\<And>F. F \<in> EF \<Longrightarrow> X \<in> F"
    by (meson A0 Pow_iff Pow_top filter_greatestD2 filter_pow_memD greatest_iff)
  then show ?thesis
    by simp
qed
...
lemma set_filter_inter:
  ...
  apply(rule filterI2)
  apply (simp add: cinf_sinf clatD2 pow_is_clattice)
  apply (metis A0 emptyE local.A1 set_filter_inter_ne)
  apply (simp add: A0 filter_inter_sub2 local.A1 pow_ne_iff2)
  apply (meson A0 cinf_sinf clatD2 filter_inter_dwdir1 is_finf_closed_def local.A1 pow_is_clattice pow_neI)
  by (simp add: A0 filter_inter_upcl2)
...
lemma pfilter_nem:"pfilter (Pow X) F \<Longrightarrow> {} \<notin> F"by (simp add: is_pfilter_def sets_pfilter_emp)
        
lemma set_pfilter_inter:
  assumes A0:"EF \<subseteq> pfilters_on (Pow X)" and A1:"EF \<noteq> {}"
  shows "is_pfilter (Pow X) (\<Inter>EF)"
  proof(rule is_pfilterI2)
  show "is_least (Pow X) {}" using pwr_least by auto
  show "{} \<notin> \<Inter> EF" using A0 A1 pfilter_nem by blast
  show "is_filter (Pow X) (\<Inter> EF)" using A0 A1 set_filter_inter[of EF X] filters_on_iff by fastforce 
qed
...
proof-
  have B0:"\<forall>G \<in>  pfilters_on (Pow X). {} \<notin> G"  by (simp add: pfilter_nem) 
  ...
  proof-
    ...
    proof
      ...
      proof
        ...
        proof-
          ...
          have B6:"\<forall>g \<in> G. g \<inter> (X-A) \<noteq> {}"   by (metis (no_types, lifting) B2 B5 Int_Collect Int_iff PowD local.A3 pfilters_sets_comp3)
          have B7:"is_pfilter (Pow X) G"   using B2 is_pfilter_def by blast
          ...
          have B14a:"H \<in> pfilters_on (Pow X) \<and>  (G, x) \<in> q \<and> G \<subseteq> H"    using B10 B3 B9 is_pfilter_def by blast
          then have B14:"(H, x) \<in> q"  using A0 by blast
          show "x \<in> Cl B"  using B13 B14 B9 Cl_def L is_pfilter_def by auto
        qed
    ...
    next
      ...
      proof
        ...
        then obtain B3:"A \<union> B \<in> G" using A3 A4 sets_filter_upc sup.cobounded2 by fastforce
        then show "x \<in> ?lhs"   by (smt (verit, del_insts) B0 Cl_def CollectD CollectI Pow_iff filter_memI le_sup_iff local.A3 local.A4 semilattice_sup_class.sup_ge1 sup_commute)  
      qed
    qed
  qed
qed
...
lemma meshI:
  "(\<And> a b. a \<in> A \<Longrightarrow> b \<in> B \<Longrightarrow> a \<inter> b \<noteq> {}) \<Longrightarrow> A # B"
  by (simp add: mesh_def)

lemma mesh_iff:
  "(A # B) \<longleftrightarrow> (\<forall>a \<in> A. \<forall>b \<in> B. a \<inter> b \<noteq> {})"
...
lemma upcl_meet:
  assumes fam:"A \<subseteq> Pow X" and upcl:"is_ord_cl (Pow X) A (\<subseteq>)" and bsub:"b \<in> Pow X" and notin:"b \<notin> A" and ain:"a \<in> A "
...
lemma isotone_mesh:
  assumes fam:"A \<subseteq> Pow X" and upcl:"is_ord_cl (Pow X) A (\<subseteq>)" and st:"E \<in> Pow X"  
  shows 11371:"E \<notin> A \<longleftrightarrow> {X-E}#A" and 11372:"E \<in> A \<longleftrightarrow> \<not> ({X-E}#A)"
  apply (metis "1134" Int_commute PowI fam mesh_singleI st upcl upcl_meet)
  by (metis "1134" Int_commute PowI fam mesh_singleI st upcl upcl_meet)


lemma notin_mesh:
  assumes fam:"A \<subseteq> Pow X" and st:"b \<in> Pow X"  and m:"{X-b} # A"
  shows "b \<notin> A"
  using assms by(auto simp add:mesh_def)

lemma mesh_iff_grill:
  assumes fam1:"A \<subseteq> Pow X" and  fam2:"B \<subseteq> Pow X"
  shows "A#B \<longleftrightarrow> A \<subseteq> grill (Pow X) B"
proof-
  have "A#B \<longleftrightarrow> (\<forall>a \<in> A. \<forall>b \<in> B. a \<inter> b \<noteq> {})" by(simp add:mesh_iff)
...
proof-
  ...
  then show "E \<in> grill (Pow X) A \<longleftrightarrow> (X-E) \<notin> A" using "11391"[of A X "X-E"] fam upcl st  by (metis Pow_iff Pow_top double_diff)
...
lemma grill_reform3:
  assumes fam:"A \<subseteq> Pow X" and upcl:"is_ord_cl (Pow X) A (\<subseteq>)" 
