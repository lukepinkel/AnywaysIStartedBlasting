...
lemma ub_set_min0:
  ...
  by (simp add: is_min_iff singleton_in_lb_set ub_set_imp)
...
lemma inf_subset_eq0:
  ...
  shows "is_inf i A B"
  by (meson A1 A2 A3 is_inf_in_iff is_inf_in_imp2 is_max_subset lb_set_in_isotone2 lb_set_mem_iff)

lemma sup_subset_eq0:
  assumes A0:"A \<subseteq> B" and A1:"B \<subseteq> C" and A2:"is_sup s A C" and A3:"s \<in> B"
  shows "is_sup s A B"
  by (meson A1 A2 A3 is_min_subset is_sup_in_iff is_sup_in_imp1 ub_set_if ub_set_imp1 ub_set_in_isotone2)
...
proof-
  ...
  proof 
    ...
    have B3:"\<forall>x \<in> Ai. x \<le> u"
      using B2 assms ub_set_mem by fastforce
    show "s \<le> u"
...
lemma principal_filter_if1:
  ...
  apply (metis insert_iff is_max_singleton is_min_sanity_check lb_def ub_set_max ub_set_min0)
...
proof
 ...
 next
  ...
  have B6:"is_min x ( up_cl {x} X)"
    by (simp add: B5 ub_set_min0 up_cl_ub)
...
proof-
  ...
  proof
    ...
    have B31:"i \<in> C \<and> i lb ?P"
      by (metis A3 B0 B30 PowI has_inf_in_set is_inf_in_imp1 is_inf_inf_eq lb_set_imp1 pow_ne_if ub_set_subset)
    have B32:"x \<le> i"
...
proof
  ...
  proof(cases "A={}")
    case True
    then show ?thesis
      by (metis A0 A3 B0 assms complete_clr1 inf_in_degenerate2 inf_subset_eq is_clr_def is_complete_imp_max_and_inf subset_empty)
  next
    case False
    then show ?thesis
