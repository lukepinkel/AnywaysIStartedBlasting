...
proof
  ...
  proof(rule congI1)
    ...
    then show "(x1 \<cdot> y1, x2 \<cdot> y2) \<in> R"
      by (metis A0 A1 Algebrah10.semigroup_def equivalence_relation.p_equivalence magma.closed px12 py12 x1 x2 y1 y2)
  qed
next
  assume A2:"cong X f R"
...
proof-
  have "monoid H (\<cdot>) e"
    by (simp add: A0 Algebrah10.group.axioms(1) subgroup_imp_group)
  then show ?thesis
