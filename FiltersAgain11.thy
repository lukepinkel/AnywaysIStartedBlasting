theory FiltersAgain11
  imports Main
begin

(*\strikethrough{6th7th8th9th10}11th times the charm*)

(*
TODO

(0). I shat the bed and ended up referring to both closure ranges as  moore closures in addition
     to the real thing.  Gotta go through and ranem
...
proof-
  fix x::"'a::order" assume A0:"has_inf (principal_filter x)"
  have B0:"x \<in> lower_bounds (principal_filter x)"
    by (simp add: imp_in_lower_bounds principal_filter_def)
  have B1:"\<forall>l \<in> lower_bounds (principal_filter x). l \<le> x"
    by (simp add: in_lower_bounds_imp principal_filter_def)
...
proof-
  have L:"\<forall>y. y \<in> principal_filter x \<longleftrightarrow> x \<le> y"
    by (simp add: principal_filter_def)
  have R:"\<forall>y. y \<in> upper_bounds {x} \<longleftrightarrow>  x \<le> y"
    by (metis in_upper_bounds_imp singleton_iff upper_bound_if)
  show ?thesis
    using L R by auto
qed
...
proof-
  have L:"\<forall>y. y \<in> principal_filter_in x A \<longleftrightarrow> x \<le> y \<and> y \<in> A"
    using principal_filter_def principal_filter_in_def by fastforce
  have R:"\<forall>y. (y \<in> upper_bounds {x} \<inter> A) \<longleftrightarrow>  x \<le> y \<and> y \<in> A"
    using principal_filter_def principal_filter_is_singleton_ub by auto
  show ?thesis
    using L R by auto
qed
...
proof
  assume L:"?L"
  show "?R"
    by (simp add: L moore_family_imp)
  next
  assume R:"?R"
  show "?L"
    by (simp add: R assms is_moore_family_def)
qed
...
lemma moore_closure_topped:
  ...
  shows "top \<in> C"
  by (metis A0 A1 Diff_disjoint Diff_empty Diff_insert0 Int_commute is_moore_family_def
       least_imp_not_empty principal_filter_in_def top_principal_filter)
...
proof-
  ...
  proof(cases "A = {}")
    case True
    have B0:"U = UNIV"
      by (simp add: True is_upper_bound_def upper_bounds_def U_def)
    have B1:"\<forall>x \<in> U. s \<le> x"
      using B0 CInf_lower U_def s_def by blast
    have B2:"s \<in> U "
      by (simp add: B0)
    ...
    then show ?thesis
      using B4 U_def is_sup_sup_eq s_def by fastforce
  next
    case False
    have B6:"\<forall>a \<in> A. \<forall>u \<in> U. a \<le> u"
      using U_def in_upper_bounds_imp by blast
    have B7:"\<forall>a \<in> A. a \<in> lower_bounds U"
      by (simp add: B6 lower_bound_if)
    have B8:"\<forall>a \<in> A. a \<le> s"
      by (simp add: B6 CInf_greatest s_def)
    have B9:"s \<in> U"
      using B8 upper_bound_if U_def by blast
    have B10:"\<forall>u \<in> U. s \<le> u"
      by (simp add: CInf_lower U_def s_def)
    have B11:"is_least s U"
      by (simp add: B10 B9 is_least_if lower_bound_if)
    have B12:"has_sup A"
      using B11 U_def has_sup_def by blast
    then show ?thesis
      using B11 U_def has_sup_imp1 is_sup_imp_lub1 is_sup_sup_eq least_unique1 s_def by blast
  qed
  show ?thesis
    using P0 by auto
qed
...
proof-
  ...
  proof(cases "A = {}")
    case True
    have B0:"L = UNIV"
      by (simp add: True is_lower_bound_def lower_bounds_def L_def)
    have B1:"\<forall>x \<in> L. x \<le> i"
      using B0 CSup_upper L_def i_def by blast
    have B2:"i \<in> L"
      by (simp add: B0)
    ...
    then show ?thesis
      using B4 L_def is_inf_inf_eq i_def by fastforce
  next
    case False
    have B6:"\<forall>a \<in> A. \<forall>l \<in> L. l \<le> a"
      using L_def in_lower_bounds_imp by blast
    have B7:"\<forall>a \<in> A. a \<in> upper_bounds L"
      by (simp add: B6 upper_bound_if)
    have B8:"\<forall>a \<in> A. i \<le> a"
      by (simp add: B6 CSup_least i_def)
    have B9:"i \<in> L"
      using B8 lower_bound_if L_def by blast
    have B10:"\<forall>l \<in> L. l \<le> i"
      by (simp add: CSup_upper L_def i_def)
    have B11:"is_greatest i L"
      by (simp add: B10 B9 is_greatest_if upper_bound_if)
    have B12:"has_inf A"
      using B11 L_def has_inf_def by blast
    then show ?thesis
      using B11 L_def has_inf_imp1 is_inf_imp_glb1 is_inf_inf_eq greatest_unique1 i_def by blast
  qed
  show ?thesis
    using P0 by auto
qed
...
proof-
  ...
  have B4:"x \<in> lower_bounds (principal_filter_in x C)"
    by (simp add: lower_bound_if principal_filter_in_imp)
...
proof-
  obtain m where A1:"is_least m A"
    using assms has_least_def by auto
  have B0:"m \<in> lower_bounds A"
...
proof-
  obtain m where A1:"is_greatest m A"
    using assms has_greatest_def by auto
  have B0:"m \<in> upper_bounds A"
    using A1 is_greatest_imp by blast
  have B1:"\<forall>u \<in> upper_bounds A. m \<le> u"
    using A1 in_upper_bounds_imp is_greatest_imp by blast
  have B2:"m = Supp A"
    by (metis B0 B1 has_sup_def is_least_def is_lower_bound_def is_sup_imp_lub3 is_sup_imp_ub1 
        is_sup_sup_eq order_antisym sup_unique2)
  show ?thesis
    using A1 B2 is_greatest_eq_if2 by auto
qed
...
proof-
  ...
  proof
    ...
    have C0B0:"\<exists>m \<in> ?Px. (\<forall>x \<in>?Px. m \<le> x)"
      by (metis assms has_least_def is_least_def is_lower_bound_def is_moore_family_def)
    have C0B1:"(?f x) = Infp(?Px)"
      by (simp add: moore_to_closure_def)
    obtain m where C0B2:"m \<in> ?Px \<and> (\<forall>x \<in>?Px. m \<le> x)"
      using C0B0 by blast
    have C0B3:"m= Infp(?Px)"
      by (metis C0B2 has_least_def has_least_imp_inf_eq_least is_least_def is_least_eq_if1 
          is_lower_bound_def least_unique2)
...
proof-
  ...
  proof
    fix x1 x2::"'a::order" assume C10A0:"x1 \<le> x2"
    ...
    have C10B1:"Infp ?Px1 \<le> Infp ?Px2"
      by (simp add: C10B0 assms has_least_imp_inf_eq_least least_anti moore_family_imp)
    show "(?f x1) \<le> (?f x2)"
      by (simp add: C10B1 complete_semilattice_inf_infp_eq moore_to_closure_def)
  qed
  show ?thesis
...
proof-
  ...
  have RtL:"(\<forall>x y. (x \<in> X \<and> y \<in> X) \<longleftrightarrow> (inf x y) \<in> X)\<longrightarrow> is_filter X"
    by (metis FiltersAgain11.is_filter_def assms binf_def downdir_up_pisystem inf.orderE is_inhabited_def 
