theory FiltersAgain12
  imports Main
begin

(*\strikethrough{6th7th8th9th10th11th}12th times the charm last one I swear*)
...
section CompleteSemilatticeClasses
...
subsection GeneralPropertiesOnPosets
...
section AlternativeOrderFunctions
subsection Bounds
...
subsubsection DescriptiveFunctions

definition InfIn::"'a::ord set \<Rightarrow> 'a::ord set \<Rightarrow> 'a" where
  "InfIn A X \<equiv> (THE i. is_inf_in i A X)"

definition SupIn::"'a::ord set \<Rightarrow> 'a::ord set  \<Rightarrow> 'a" where
  "SupIn A X \<equiv> (THE s. is_sup_in s A X)"

definition Infp::"'a::ord set \<Rightarrow> 'a::ord" where
  "Infp A \<equiv> (THE i. is_inf i A)"

definition Supp::"'a::ord set \<Rightarrow> 'a::ord" where
  "Supp A \<equiv> (THE s. is_sup s A)"


definition Greatest::"'a::ord set \<Rightarrow> 'a::ord" where
  "Greatest A \<equiv> (THE m. is_greatest m A)"

definition Least::"'a::ord set \<Rightarrow> 'a::ord" where
  "Least A \<equiv> (THE m. is_least m A)"

definition binf::"'a::ord  \<Rightarrow> 'a::ord \<Rightarrow> 'a::ord" where
  "binf a b \<equiv> Infp {a,b}"

definition bsup::"'a::ord \<Rightarrow> 'a::ord \<Rightarrow> 'a::ord" where
  "bsup a b \<equiv> Supp {a, b}"

definition is_join_dual::"('a::ord \<Rightarrow> 'b::ord) \<Rightarrow> bool" where
  "is_join_dual f \<equiv> (\<forall>A. ( (f (Supp A)) = (Infp (f`(A))) ))"

definition join_dual_partner::"('a::ord \<Rightarrow> 'b::ord) \<Rightarrow> ('b::ord \<Rightarrow> 'a::ord)" where
  "join_dual_partner f = (\<lambda>y::('b::ord). Supp {x::'a::ord. y \<le> (f x)})"


section SubsetsOfPosets
subsection GeneralPredicates
...
definition is_pisystem::"'a::ord set \<Rightarrow> bool" where
...
definition is_compactly_generated::"'a::ord set  \<Rightarrow> bool" where
  "is_compactly_generated X \<equiv> (\<forall>x \<in> X. (\<exists>Cx. (\<forall>c \<in> Cx. is_compact c) \<and> (is_sup x Cx)))"

definition is_meet_reducible::"'a::ord \<Rightarrow> bool" where
  "is_meet_reducible x \<equiv> (\<exists>A. (A \<noteq> {}) \<and> (x \<notin> A) \<and> (is_inf x A))"

definition is_join_reducible::"'a::ord \<Rightarrow> bool" where
  "is_join_reducible x \<equiv> (\<exists>A. (A \<noteq> {}) \<and> (x \<notin> A) \<and> (is_sup x A))"

subsection MooreCollectionsAndClosures
definition is_moore_family::"'a::ord set \<Rightarrow> bool" where
  "is_moore_family C \<equiv> (C \<noteq> {}) \<and> (\<forall>(a::'a). has_least (principal_filter_in a C))"

definition moore_to_closure::"'a::ord set \<Rightarrow> ('a::ord \<Rightarrow> 'a::ord)" where
  "moore_to_closure C \<equiv> (\<lambda>x. Infp(principal_filter_in x C))"
...
lemma principal_filter_iff:
  "\<And>y x. y \<in> principal_filter x \<longleftrightarrow> x \<le> y"
  by (simp add: principal_filter_def)

lemma upper_bound_singleton_iff:
  "\<And>y x. y \<in> upper_bounds {x}  \<longleftrightarrow> x \<le> y"
  by (metis imp_in_upper_bounds in_upper_bounds_imp singleton_iff)

lemma lower_bound_singleton_iff:
  "\<And>y x. y \<in> lower_bounds {x}  \<longleftrightarrow> x \<ge> y"
  by (metis imp_in_lower_bounds in_lower_bounds_imp singleton_iff)

subsubsection ExtremaPredicates

lemma is_greatest_imp:
  "\<And>g A. is_greatest g A \<Longrightarrow> g \<in> A \<and> g \<in> upper_bounds A"
  by (simp add: is_greatest_def upper_bounds_def)

lemma is_least_imp:
  "\<And>l A. is_least l A \<Longrightarrow> l \<in> A \<and> l \<in> lower_bounds A"
  by (simp add: is_least_def lower_bounds_def)


lemma is_greatest_imp2:
  "\<And>g A. is_greatest g A \<Longrightarrow> (\<And>a. a \<in> A \<Longrightarrow> a \<le> g)"
  by (simp add: is_greatest_def is_upper_bound_def)

lemma is_least_imp2:
  "\<And>l A. is_least l A \<Longrightarrow> (\<And>a. a \<in> A \<Longrightarrow> l \<le> a)"
  by (simp add: is_least_def is_lower_bound_def)

lemma is_greatest_if:
  "\<And>g A.  g \<in> A \<and> g \<in> upper_bounds A \<Longrightarrow> is_greatest g A"
  by (simp add: is_greatest_def upper_bounds_def)

lemma is_least_if:
  "\<And>l A.  l \<in> A \<and> l \<in> lower_bounds A \<Longrightarrow> is_least l A"
  by (simp add: is_least_def lower_bounds_def)


lemma greatest_imp_not_empty:
  "\<And>A. has_greatest A \<Longrightarrow> A \<noteq> {}"
  using has_greatest_def is_greatest_imp by auto

lemma least_imp_not_empty:
  "\<And>A. has_least A \<Longrightarrow> A \<noteq> {}"
  using has_least_def is_least_imp by auto



subsubsection GreatestLeastUniqueness

lemma greatest_unique1:
  "\<And>(A::'a::order set) m1 m2. is_greatest m1 A \<Longrightarrow> is_greatest m2 A \<Longrightarrow> m1 = m2 "
  by (simp add: is_greatest_def is_greatest_imp2 order_antisym)

lemma greatest_unique2:
  "\<And>(A::'a::order set). has_greatest A \<Longrightarrow> (\<exists>!m. is_greatest m A) "
  by (meson greatest_unique1 has_greatest_def)

lemma least_unique1:
  "\<And>(A::'a::order set) m1 m2. is_least m1 A \<Longrightarrow> is_least m2 A \<Longrightarrow> m1 = m2 "
  by (simp add: is_least_def is_least_imp2 order_antisym)

lemma least_unique2:
  "\<And>(A::'a::order set). has_least A \<Longrightarrow>(\<exists>!m. is_least m A)"
  using has_least_def least_unique1 by blast

subsubsection ExtremaOperators

lemma is_greatest_eq_if1:
  "\<And>(A::'a::order set). has_greatest A \<Longrightarrow> is_greatest (Greatest A) A"
  by (metis Greatest_def greatest_unique2 theI)

lemma is_least_eq_if1:
  "\<And>(A::'a::order set). has_least A \<Longrightarrow> is_least (Least A) A"
  by (metis Least_def least_unique2 theI)

lemma is_greatest_eq_if2:
  "\<And>(A::'a::order set) g. is_greatest g A \<Longrightarrow>  g = (Greatest A)"
  using greatest_unique1 has_greatest_def is_greatest_eq_if1 by blast

lemma is_least_eq_if2:
  "\<And>(A::'a::order set) l. is_least l A \<Longrightarrow> l =  (Least A)"
  using has_least_def is_least_eq_if1 least_unique1 by blast
...
lemma is_inf_imp_glb2:
  ...
  using is_greatest_imp2 is_inf_imp_glb1 by blast


lemma is_inf_imp_glb3:
  "\<And>l i A. is_inf i A \<Longrightarrow> l \<in> lower_bounds A \<Longrightarrow> l \<le> i"
...
lemma exists_inf_in_is_lb:
  "has_inf_in (A::'a::order set) X \<Longrightarrow> (InfIn A X) \<in> lower_bounds_in A X"
  by (metis InfIn_def glb_imp_is_inf_in greatest_unique1 has_inf_in_def is_inf_in_imp_glb1 is_inf_in_imp_lb1 the_equality)

lemma exists_inf_is_inf_glb:
  "has_inf_in (A::'a::order set) X \<Longrightarrow> (is_greatest (InfIn A X) (lower_bounds_in A X))"
  by (metis InfIn_def glb_imp_is_inf_in greatest_unique1 has_inf_in_def is_inf_in_imp_glb1 the_equality)

lemma exists_sup_in_is_ub:
  "has_sup_in (A::'a::order set) X \<Longrightarrow> (SupIn A X) \<in> upper_bounds_in A X"
  by (metis SupIn_def lub_imp_is_sup_in least_unique1 has_sup_in_def is_sup_in_imp_lub1 is_sup_in_imp_ub1 the_equality)

lemma exists_sup_in_is_lub:
  "has_sup_in (A::'a::order set) X \<Longrightarrow> (is_least (SupIn A X) (upper_bounds_in A X))"
  by (metis SupIn_def lub_imp_is_sup_in least_unique1 has_sup_in_def   is_sup_in_imp_lub1 the_equality)
...
lemma greatest_iso:
  ...
  using is_greatest_eq_if1 is_greatest_imp is_greatest_imp2 by blast
...
lemma empty_lb:
  "UNIV = lower_bounds {}"
  by (simp add: UNIV_eq_I imp_in_lower_bounds)

lemma empty_ub:
  "UNIV = upper_bounds {}"
  by (simp add: UNIV_eq_I imp_in_upper_bounds)

lemma top_is_greatest:
  fixes top::"'a::order"
  assumes "\<forall>x. x \<le> top"
  shows "is_greatest top UNIV"
  by (simp add: assms is_greatest_def is_upper_bound_def)

lemma bot_is_least:
  fixes bot::"'a::order"
  assumes "\<forall>x. bot \<le> x"
  shows "is_least bot UNIV"
  by (simp add: assms is_least_def is_lower_bound_def)
...
proof 
  ...
  next
  assume A1:" (\<forall>x1 x2. ((x1 \<le> (f x2)) \<longleftrightarrow>( (f x1) \<le> (f x2))))"
  have C0:"\<And>x1 x2. x1 \<le> x2 \<longrightarrow> (f x1) \<le> (f x2)"
  proof
    fix x1 x2 assume C01:"(x1::('a::order)) \<le> (x2::('a::order))"
    have B2:"x1 \<le> (f x2)"
...
subsubsection PointwiseOrder
lemma pw_less_eq_imp:
  fixes f g::"('a::order \<Rightarrow> 'b::order)"
  shows "\<And>f g. pointwise_less_eq f g \<Longrightarrow> (\<And>x. f x \<le> g x)"
...
lemma pw_less_antisym:
  ...
  shows "f = g"
  apply (simp add:fun_eq_iff assms order_antisym pointwise_less_eq_def  Orderings.order_eq_iff)
  by (simp add: assms pw_less_eq_imp)
...
lemma principal_filter_order_iso:
  "\<And>(x::'a::order) y::'a::order. x \<le> y \<Longrightarrow> (principal_filter y) \<subseteq> (principal_filter x) "
  by (metis atLeast_def atLeast_subset_iff principal_filter_def)


lemma principal_filter_in_order_iso:
  "\<And>(x::'a::order) (y::'a::order) (C::('a::order set)). x \<le> y \<Longrightarrow> (principal_filter_in y C) \<subseteq> (principal_filter_in x C) "
  by (simp add: inf.coboundedI2 principal_filter_in_def principal_filter_order_iso)


subsubsection InfSupUniqueness

lemma inf_in_unique1:
  "\<And>(A::'a::order set) X i1 i2. is_inf_in i1 A X \<Longrightarrow> is_inf_in i2 A X \<Longrightarrow> i1 = i2"
  by (simp add: is_inf_in_imp_glb3 is_inf_in_imp_lb1 order_antisym)

lemma inf_unique1:
  "\<And>(A::'a::order set) i1 i2. is_inf i1 A \<Longrightarrow> is_inf i2 A \<Longrightarrow> i1 = i2"
  by (simp add: is_inf_imp_glb3 is_inf_imp_lb1 order_antisym)


lemma sup_in_unique1:
  "\<And>(A::'a::order set) X s1 s2. is_sup_in s1 A X \<Longrightarrow> is_sup_in s2 A X \<Longrightarrow> s1 = s2"
  by (simp add: is_sup_in_imp_lub3 is_sup_in_imp_ub1 order_antisym)

lemma sup_unique1:
  "\<And>(A::'a::order set) s1 s2. is_sup s1 A \<Longrightarrow> is_sup s2 A \<Longrightarrow> s1 = s2"
  by (simp add: is_sup_imp_lub3 is_sup_imp_ub1 order_antisym)


lemma has_inf_in_imp1:
  "\<And>(A::'a::order set) X. has_inf_in A X \<Longrightarrow> (\<exists>i. is_inf_in i A X)"
  by (simp add: has_inf_in_def  is_inf_in_def)
...
lemma inf_in_unique2:
  "\<And>(A::'a::order set) X. has_inf_in A X \<Longrightarrow> (\<exists>!i. is_inf_in i A X)"
  using has_inf_in_imp1 inf_in_unique1 by blast

lemma inf_unique2:
  "\<And>(A::'a::order set). has_inf A \<Longrightarrow> (\<exists>!i. is_inf i A)"
  using has_inf_imp1 inf_unique1 by blast


lemma has_sup_in_imp1:
  "\<And>(A::'a::order set) X. has_sup_in A X \<Longrightarrow> (\<exists>s. is_sup_in s A X)"
  by (simp add: has_sup_in_def is_sup_in_def)
...
lemma sup_in_unique2:
  "\<And>(A::'a::order set) X. has_sup_in A X \<Longrightarrow> (\<exists>!s. is_sup_in s A X)"
  using has_sup_in_imp1 sup_in_unique1 by blast

lemma sup_unique2:
  "\<And>(A::'a::order set). has_sup A \<Longrightarrow> (\<exists>!s. is_sup s A)"
  using has_sup_imp1 sup_unique1 by blast

subsubsection InfSupOperators

lemma is_inf_inf_eq:
  "\<And>(i::'a::order) A. has_inf A \<Longrightarrow> is_inf i A \<Longrightarrow> (i = Infp A)"
  by (simp add: Infp_def inf_unique2 the1_equality)

lemma is_sup_sup_eq:
  "\<And>(s::'a::order) A. has_sup A \<Longrightarrow> is_sup s A \<Longrightarrow> (s = Supp A)"
...
lemma sup_in_space_anti:
  ...
  by (meson A0 A2 A3 exists_sup_in_is_lub exists_sup_in_is_ub is_sup_in_def is_sup_in_imp_lub3 subset_iff upper_bounds_in_iso2)  
...
lemma inf_in_space_mono:
  ...
  by (meson A0 A2 A3 exists_inf_in_is_lb exists_inf_is_inf_glb in_mono is_greatest_imp2 lower_bounds_in_iso2)            

  

subsubsection PrincipalFilterImplications

lemma principal_inf0:
  "\<And>x::('a::order). has_inf (principal_filter x) \<Longrightarrow> x = Infp (principal_filter x)"
  by (metis dual_order.refl inf_unique2 is_inf_imp_glb3 is_inf_imp_lb1 is_inf_inf_eq 
      is_least_eq_if2 is_least_if is_lower_bound_def lower_bounds_def mem_Collect_eq principal_filter_def)


lemma principal_inf1:
  "\<forall>x::('X::complete_semilattice_inf). x = (Inf(principal_filter x))"
  by (metis CInf_greatest CInf_lower CollectI order_class.order_eq_iff principal_filter_def principal_filter_imp)



lemma principal_filter_is_singleton_ub:
  fixes x::"'a::order"
  shows"(principal_filter x) = upper_bounds {x}"
  by (meson order_antisym_conv principal_filter_iff subsetI upper_bound_singleton_iff)


lemma principal_filter_in_is_singleton_ub:
  fixes x::"'a::order" and A::"'a::order set"
  shows"(principal_filter_in x A) = upper_bounds {x} \<inter> A"
  by (simp add: Int_commute principal_filter_in_def principal_filter_is_singleton_ub)
...
lemma is_lub_singleton:
  "is_least (x::'a::order) (upper_bounds {x})"
  by (simp add: is_sup_imp_lub1 is_sup_singleton)
...
lemma moore_family_imp:
  "\<And>C. is_moore_family C \<Longrightarrow>(\<And>a. has_least (principal_filter_in a C))"  
...
lemma moore_family_if_lpf:
  assumes "C \<noteq> {}"
  shows "is_moore_family C \<longleftrightarrow> (\<forall>a. has_least (principal_filter_in a C))" (is "?L \<longleftrightarrow> ?R")
  by (simp add: assms is_moore_family_def)

lemma top_principal_filter:
  fixes top::"'a::order"
  assumes is_top:"\<forall>x. x \<le> top"
  shows "principal_filter top = {top}"
  by (simp add: is_top order_antisym_conv principal_filter_def)

lemma top_least_in_principal_filter:
  fixes top::"'a::order"
  assumes is_top:"\<forall>x. x \<le> top"
  shows "is_least top (principal_filter top)"
  by (simp add: is_least_def is_lower_bound_def principal_filter_def)
...
lemma moore_closure_topped:
  ...
  shows "top \<in> C"
  by (metis A0 A1 Int_empty_right Int_insert_right_if0 is_moore_family_def least_imp_not_empty principal_filter_in_def top_principal_filter)
...
proof-
  ...
  have B1:"\<forall>x \<in> E. i \<le> x"
    using A2 is_inf_imp_lb2 by blast
  ...
  have B5:"f i \<le> i"
    using A2 B4 is_inf_imp_glb3 lower_bound_if by blast
  have B6:"f i = i"
...
lemma moore_closure_imp:
  ...
  shows "\<forall>x. (moore_to_closure C) x = Infp(principal_filter_in x C)"
...
lemma moore_family_is_cofinal:
  ...
  by (simp add: assms least_imp_not_empty moore_family_imp)
...
lemma complete_semilattice_inf_is_inf:
  ...
  by (meson CInf_greatest CInf_lower glb_imp_is_inf in_lower_bounds_imp is_greatest_if lower_bound_if upper_bound_if)
...
proof-
  ...
  have B1:"is_greatest i (lower_bounds_in A X)"
    by (simp add: CInf_greatest CInf_lower assms i_def imp_in_upper_bounds in_lower_bounds_in_imp is_greatest_if lower_bound_in_if)
  show ?thesis
    using B1 exists_inf_is_inf_glb glb_imp_is_inf_in has_inf_in_def by blast
qed
...
proof-
  ...
  have B1:"is_least s (upper_bounds_in A X)"
    by (simp add: CSup_least CSup_upper assms s_def imp_in_lower_bounds in_upper_bounds_in_imp is_least_if upper_bound_in_if)
  show ?thesis
    using B1 exists_sup_in_is_lub lub_imp_is_sup_in has_sup_in_def by blast
qed
...
lemma complete_semilattice_inf_infp_eq:
  ...
  by (metis complete_semilattice_inf_has_inf complete_semilattice_inf_is_inf is_inf_inf_eq)

lemma complete_semilattice_sup_is_sup:
  "\<forall>(A::'a::complete_semilattice_sup set). is_sup (Sup A) A"
  by (metis complete_semilattice_sup_in_eq_sup_in complete_semilattice_sup_in_is_sup_in is_sup_def iso_tuple_UNIV_I)
...
lemma complete_semilattice_sup_supp_eq:
  ...
  by (metis complete_semilattice_sup_has_sup complete_semilattice_sup_is_sup is_sup_sup_eq)
...
lemma complete_semilattice_inf_ub_imp_sup:
  ...
  shows "has_sup A \<and> Supp A = Inf(upper_bounds A)"
proof-
  define U where "U=upper_bounds A"
  define s where "s=Inf(U)"
  have P0:"has_sup A \<and> Supp A = Inf(upper_bounds A)"
  proof(cases "A = {}")
    case True then show ?thesis
      by (metis CInf_lower UNIV_I bot_is_least empty_ub has_sup_def is_least_eq_if2 supp_is_lub)
  next
    case False then show ?thesis
      by (metis CInf_greatest complete_semilattice_inf_is_inf has_sup_def in_upper_bounds_imp
           is_inf_imp_lb1 is_least_eq_if2 is_least_if supp_is_lub upper_bound_if)
  qed
  show ?thesis
    using P0 by auto
qed
...
lemma complete_semilattice_sup_lb_imp_inf:
  ...
  shows "has_inf A \<and> Infp A = Sup(lower_bounds A)"
proof-
  define L where "L=lower_bounds A"
  define i where "i=Sup(L)"
  have P0:"has_inf A \<and> Infp A = Sup(lower_bounds A)"
  proof(cases "A = {}")
    case True then show ?thesis
      by (metis CSup_upper UNIV_I empty_lb glb_imp_is_inf has_inf_def is_inf_inf_eq top_is_greatest)
  next
    case False 
    then show ?thesis
      by (metis complete_semilattice_sup_is_sup has_inf_def infp_is_gbl is_greatest_eq_if2 
          is_greatest_if is_least_imp is_sup_reduce lower_bounds_iso lower_upper_comp_extensive
             order_antisym upper_lower_comp_extensive)
  qed
  show ?thesis
    using P0 by auto
qed
...
proof
  ...
  have B1:"\<forall>u \<in> upper_bounds_in A B. s \<le> u"
  ...
  have B3:"is_sup_in s A B"
    by (simp add: B0 B1 is_least_def is_lower_bound_def lub_imp_is_sup_in)
  show "?R"
...
proof
  ...
  have B1:"\<forall>l \<in> lower_bounds_in A B. l \<le> i"
  ...
  have B3:"is_inf_in i  A B"
    by (simp add: B0 B1 glb_imp_is_inf_in is_greatest_def is_upper_bound_def)
  show "?R"
...
lemma principal_inf2:
  ...
  assumes A0:"x \<in> C"
  shows "x = Infp(principal_filter_in x C)"
  by (metis CInf_lower IntI assms complete_semilattice_inf_infp_eq 
      complete_semilattice_inf_is_inf is_inf_imp_glb3 lower_bound_if order_antisym 
      principal_filter_iff principal_filter_in_def principal_filter_in_imp principal_inf1)
...
proof-
  ...
  show ?thesis
    by (metis B0 B2 CInf_lower Pow_iff empty_iff empty_subsetI has_least_def 
        is_least_if lower_bound_if moore_family_if_lpf order_antisym)
qed  
...
lemma moore_closure_imp_csemilattice_inf:
  ...
  by (simp add: complete_semilattice_inf_infp_eq moore_to_closure_def)
...
lemma complete_lattice_inf_is_inf:
  ...
  by (meson csli.CInf_greatest csli.CInf_lower glb_imp_is_inf in_lower_bounds_imp is_greatest_if 
      lower_bound_if upper_bound_if)


lemma complete_lattice_sup_is_sup:
  fixes A::"'a::complete_lattice set"
  shows "(is_sup (Sup A) A)"
  by (meson csls.CSup_least csls.CSup_upper imp_in_lower_bounds in_upper_bounds_imp 
      is_least_if lub_imp_is_sup upper_bound_if)
...
lemma complete_semilattice_sup_least:
  ...
  by (simp add: CSup_least in_upper_bounds_imp)

lemma complete_lattice_inf_greatest:
  fixes A::"'a::complete_lattice set"
  shows "\<forall>l \<in> lower_bounds A. l \<le> Inf A"
  by (simp add: in_lower_bounds_imp le_Inf_iff)
...
lemma has_least_imp_inf_eq_least:
  fixes A::"'a::order set"
  assumes A0:"has_least A"
  shows "Infp A = Least A"
  by (metis assms glb_imp_is_inf has_inf_def is_greatest_if is_inf_inf_eq is_least_eq_if1 
      is_least_imp subset_iff upper_lower_comp_extensive)


lemma has_greatest_imp_sup_eq_greatest:
  fixes A::"'a::order set"
  assumes A0:"has_greatest A"
  shows "Supp A = Greatest A"
  by (metis assms has_sup_def in_mono is_greatest_eq_if1 is_greatest_imp is_least_if is_sup_sup_eq
       lower_upper_comp_extensive lub_imp_is_sup)
...
proof-
  ...
  have C01:"\<forall>x. x \<le> ?f x"
    by (metis assms has_least_imp_inf_eq_least is_least_def is_least_eq_if1
         is_moore_family_def moore_to_closure_def principal_filter_in_imp)
    show ?thesis
...
proof-
  ...
  have C10:"\<And>x1 x2. x1 \<le> x2 \<longrightarrow> (?f x1) \<le> (?f x2)"
    by (simp add: assms has_least_imp_inf_eq_least least_anti moore_closure_imp
         moore_family_imp principal_filter_in_order_iso)
  show ?thesis
...
proof-
  ...
  proof-
    ...
    proof 
      ...
      have C2B0:"?y1 \<in> ?Px"
        by (simp add: assms has_least_imp_inf_eq_least is_least_eq_if1 is_least_imp moore_family_imp moore_to_closure_def)
      have C2B1:"?y2 \<in> ?Pfx"
        by (simp add: assms has_least_imp_inf_eq_least is_least_eq_if1 is_least_imp moore_family_imp moore_to_closure_def)
      have C2B2:"?y1 \<in> C"
      ...
      have C2B3:"?y1 = Infp ?Pfx"
        by (metis C2B1 C2B3 C2B4 assms has_least_imp_inf_eq_least is_least_eq_if1 
            is_least_imp2 moore_family_imp moore_to_closure_def order_antisym)
      show "?f x = ?f (?f x)"
...
proof
  ...
  have B0:"f(c) = Infp(principal_filter_in c C)"
    by (simp add: assms f_def moore_closure_imp)
  have B1:"... = c"
    by (metis A1 Int_Collect assms dual_order.refl has_least_imp_inf_eq_least is_least_eq_if1 
        is_least_imp is_least_imp2 moore_family_imp order_antisym principal_filter_def principal_filter_in_def)
  show "f(c) = c"
    by (simp add: B0 B1)
qed 
...
proof
  fix c assume A1:"f(c) = c"
  have B0:"... = Infp(principal_filter_in c C)"
    by (metis A1 f_def moore_to_closure_def)
  have B1:"... = Least (principal_filter_in c C)"
    by (simp add: assms has_least_imp_inf_eq_least moore_family_imp)
  show "c \<in> C"
    by (metis A0 B0 B1 Int_iff is_least_def is_least_eq_if1 is_moore_family_def principal_filter_in_def)
qed
...
lemma moore_then_inf_closed:
  ...
  shows "\<And>A. (A \<in> (Pow C) \<and> has_inf A) \<longrightarrow> Infp A \<in> C"
proof
  fix A assume A1:"(A \<in> (Pow C) \<and> has_inf A)"
  define i where "i = Infp A"
  show "Infp A \<in> C"
  proof(cases "A = {}")
    case True
    have B0:"Infp A = top"
      by (metis A1 True empty_subsetI has_inf_def inf_anti is_glb_singleton is_inf_inf_eq
           is_inf_singleton is_top order_antisym)
    then show ?thesis
  ...
  next
    ...
    have B1:"\<forall>a \<in> A. i \<le> a \<and> a \<in> C"
      by (metis A1 PowD i_def in_mono inf_unique2 is_inf_imp_lb2 is_inf_inf_eq)
    ...
    have B4:"f i \<le> i"
      by (metis A1 B2 B3 has_inf_imp1 i_def is_inf_imp_glb3 is_inf_inf_eq lower_bound_if)
    have B5:"f i = i"
...
proof-
  ...
  proof
    ...
    qed
  have B4:"\<forall>x. (has_least (principal_filter_in x ?C))"
    by (meson B3 has_least_def imp_in_lower_bounds is_least_if)
  show ?thesis
...
proof-
  ...
  proof
    ...
    have A4:"f(a) = Infp(principal_filter_in a (range f))"
      by (metis A0 A3 has_least_def has_least_imp_inf_eq_least is_least_def is_least_eq_if2 is_lower_bound_def)
    have B1:"principal_filter_in a (range f) = (range f) \<inter> {y. a \<le> y}"
      by (simp add: principal_filter_def principal_filter_in_def)
    have B2:"(g a) = Infp(principal_filter_in a (range f))"
      by (simp add: g_def moore_to_closure_def)
    have B3:"... = Infp{y \<in> range f. a \<le> y}"
      by (simp add: B1 Collect_conj_eq)
    have B4:"... = Infp{y. \<exists>x. (y = f x) \<and> (a \<le> (f x))}"
  ...
  have B0:"\<And>y. (y \<in> (range f)) \<longrightarrow>  y \<in> C"
    proof
      ...
      obtain x where B0A1:"(f x) = y"
        using B0A0 by blast
      have B0B1:"y = Infp(principal_filter_in x C)"
...
proof-
  ...
  have B3:"\<forall>x. Infp( principal_filter_in x ?G1) \<le> Infp( principal_filter_in x ?G2)"
    by (simp add: A1 B2 clrange_is_moore has_least_imp_inf_eq_least least_anti moore_family_imp)
...
lemma moore_family_is_complete_semilattice_inf:
  ...
  shows "((has_inf_in A C) \<and> (InfIn A C = Infp A))"
proof-
  define i where "i=Inf A"
  have B0:"is_inf i A"
    by (metis complete_semilattice_inf_has_inf complete_semilattice_inf_infp_eq 
        has_inf_imp1 is_inf_inf_eq i_def)
  have B1:"i \<in> C"
  ...
  have B5:"InfIn A C  = i"
    by (simp add: B2 InfIn_def inf_in_unique1 the_equality)
  have B6: "InfIn A C = Infp A"
...
subsubsection InfAndSup
(*
  This is a little troubling - it doesnt really make sense to apply a binary operation to no elements
  so Infp neq fInf when A = \<emptyset> etc.  Thoroughly disconcerting
 *)
...
proof-
  ...
  have B0:"i \<in> lower_bounds A"
    by (simp add: assms coboundedI i_def imp_in_lower_bounds)
...
proof-
  have B0:"is_inf (Infp A) A"
    using A0 A1 has_inf_imp1 is_inf_inf_eq semilattice_inf_has_inf by blast
  have B1:"is_inf (fInf A) A"
    by (metis A0 A1 B0 coboundedI imp_in_lower_bounds is_inf_imp_glb3 is_inf_imp_lb2 lower_bounded_iff order_antisym)
  show "Infp A = fInf A"
...
proof-
  ...
  have B0:"s \<in> upper_bounds A"
    by (simp add: A0 s_def semilattice_sup_class.in_idem sup.absorb_iff2 upper_bound_if)
...
lemma semilattice_sup_suppp_eq_small_sup:
  fixes a::"'a::semilattice_sup" and b::"'a::semilattice_sup"
  shows "sup a b = Supp {a, b}"
  by (simp add: semilattice_sup_supp_eq)


lemma semilattice_sup_bsup_eq_sup:
  fixes a::"'a::semilattice_sup" and b::"'a::semilattice_sup"
  shows "sup a b = bsup a b"
  by (simp add: bsup_def semilattice_sup_supp_eq)
...
lemma lattice_inf_has_inf:
  ...
  assumes A0:"finite A" and
          A1:"A \<noteq> {}"
  shows " has_inf A"
  by (simp add: A0 A1 semilattice_inf_has_inf)


lemma lattice_inf_infp_eq:
  fixes A::"'a::lattice set"
  assumes A0:"finite A" and
          A1:"A \<noteq> {}"
  shows " Infp A = fInf A"
  by (simp add: A0 A1 semilattice_inf_infp_eq)


lemma lattice_sup_has_sup:
  fixes A::"'a::lattice set"
  assumes A0:"finite A" and
          A1:"A \<noteq> {}"
  shows " has_sup A"
  by (simp add: A0 A1 semilattice_sup_has_sup)

lemma lattice_sup_supp_eq:
  fixes A::"'a::lattice set"
  assumes A0:"finite A" and
          A1:"A \<noteq> {}"
  shows " Supp A = fSup A"
...
proof-
  ...
  show ?thesis
    by (metis B0 emptyE has_least_def is_inhabited_imp is_least_def is_lower_bound_def is_moore_family_def ne)
qed
...
proof-
  have B0:"\<forall>a \<in> A. is_lower_bound a {a}"
    by (simp add: is_lower_bound_def)
...
proof-
  ...
  have B0:"c1 \<in> lower_bounds {a, b}"
    by (simp add: c1_def imp_in_lower_bounds)
  have B1:"\<forall>l \<in> lower_bounds {a, b}. l \<le> c1"
    by (simp add: c1_def in_lower_bounds_imp)
...
proof-
  ...
  proof
     ...
     qed
  qed
  have B5:"(\<forall>(a::(('X::bounded_semilattice_inf_top set))). has_least (principal_filter_in a ?EF))"
    by (meson B4 has_least_def is_least_def is_lower_bound_def)
  show ?thesis
...
proof-
  ...
  have B7:"\<And>y A. y \<le> Infp(f`(A)) \<longleftrightarrow> (y \<le> (f (Supp A)))"
  proof
    fix y A assume A1:"y \<le> Infp(f`(A))"
    have B70:"... = Inf(f`(A))"
      by (metis complete_lattice_inf_is_inf has_inf_def is_inf_imp_glb1 is_inf_inf_eq)
    have B71:"Supp A = Sup A"
      by (metis complete_lattice_sup_is_sup has_sup_def is_sup_imp_lub1 is_sup_sup_eq)
    have B72:"(f (Supp A)) = (f((Sup A)))"
      by (simp add: B71)
    show "(y \<le> (f (Supp A)))"
      using A1 B6 B70 B71 by fastforce
    next
    fix y A assume "(y \<le> (f (Supp A)))"
    have B73:"Supp A = Sup A"
      by (metis complete_lattice_sup_is_sup has_sup_def is_sup_imp_lub1 is_sup_sup_eq)
    have B74:"(f (Supp A)) = (f((Sup A)))"
      by (simp add: B73)
    have B75:"(y \<le> (f (Sup A)))"
      using B74 \<open>(y::'Y::complete_lattice) \<le> (f::'X::complete_lattice \<Rightarrow> 'Y::complete_lattice) (Supp (A::'X::complete_lattice set))\<close> by fastforce
    have B75:"y \<le> Inf(f`(A))"
      using B6 B75 by blast
    show "y \<le> Infp(f`(A))"
      by (metis B75 complete_lattice_inf_is_inf has_inf_def is_inf_imp_glb1 is_inf_inf_eq)
  qed
  show ?thesis
...
proof-
  ...
  proof
    ...
    have B00:"Supp {x1, x2} = x2"
      by (metis A1 Sup_insert ccpo_Sup_singleton complete_lattice_sup_is_sup has_sup_def 
          is_sup_imp_lub1 is_sup_sup_eq le_iff_sup)
    have B01:"(f x2) = (f (Supp {x1, x2}))"
      by (simp add: B00)
    have B01:"... = (Infp {(f x1), (f x2)})"
  ...
  proof
    ...
    have B10:"(?g y2) = Supp ?B2"
      by (simp add: join_dual_partner_def)   
    have B11:"(?g y1) = Supp ?B1"
      by (simp add: join_dual_partner_def)   
    have B12:"?B2 \<subseteq> ?B1"
      using A2 by force
    have B13:"is_sup (Sup ?B1) ?B1 \<and>is_sup (Sup ?B1) ?B1"
      using complete_lattice_sup_is_sup by blast
    have B14:"has_sup ?B1 \<and> has_sup ?B2"
      using complete_lattice_sup_is_sup has_sup_def is_sup_imp_lub1 by blast
    have B15:"(?g y2) \<le> (?g y1)"
      by (simp add: B10 B11 B12 B14 sup_iso)
    show "(?g y2) \<le> (?g y1)"
      by (simp add: B15)
  qed
  ...
  proof-
    ...
    have B20:"?g y =  Supp {x. y \<le> (f x)}"
      by (simp add: join_dual_partner_def)
    have B21:"f (?g y) = Infp (f`{x. (f x) \<ge> y})"
      using B20 assms is_join_dual_def by auto
    have B22:"y \<in> lower_bounds (f`{x. (f x) \<ge> y})"
      by (simp add: imp_in_lower_bounds)
    have B23:"y \<le>  Infp (f`{x. (f x) \<ge> y})"
      by (metis B22 complete_lattice_inf_is_inf has_inf_def is_inf_imp_glb1 is_inf_imp_glb3 is_inf_inf_eq)
    show "y \<le> (f (?g y))"
      by (simp add: B21 B23)
  qed
  have B3:"\<And>(x::('X::complete_lattice)). x \<le> (?g (f x))"
    by (metis (full_types) complete_lattice_sup_is_sup has_sup_def is_sup_imp_lub1 is_sup_imp_ub2 is_sup_sup_eq join_dual_partner_def mem_Collect_eq order_refl)
  ...
  have B15:"\<forall>A. (\<Inter>a \<in> A. f {a}) = (f (\<Union>a \<in> A. {a}))"
    proof
      ...
      have B152:"?R= f (Sup ?sA)"
        by (simp add: image_def)
      have B153:"... = f (Supp ?sA)"
        by (metis (no_types, lifting) complete_lattice_sup_is_sup has_sup_def is_sup_imp_lub1 is_sup_sup_eq)
      have LR:"?L = ?R"
        by (smt (verit, best) B0 B153 Collect_cong INT_extend_simps(10)
             complete_lattice_inf_is_inf has_inf_def image_def is_inf_imp_glb1 is_inf_inf_eq is_join_dual_def)
...
proof-
  ...
  proof
    ...
    have B16:"is_finite_subcover S A c"
      by (meson A1 A9 covers_def in_lower_bounds_imp is_finite_subcover_def lower_bound_if subset_eq)
    ...
    assume A2:"\<not>(m < Inf {x. m < x})"
      ...
      proof-
        have B50:"Inf ?L \<in> lower_bounds ?L"
          by (meson Inf_lower lower_bound_if)
  ...
  proof-
     ...
     proof
      ...
      have B7:"\<forall>x \<in> A. m < x"
        using A5 is_inf_imp_lb2 nless_le by blast
      have B8:"m \<le> Inf {x. m < x}"
        by (simp add: A3 order_le_less)
      show " \<not>(m < Inf {x. m < x})"
        by (metis A5 B7 Inf_superset_mono complete_lattice_inf_is_inf inf_unique1 leD mem_Collect_eq subsetI)
      qed
    show ?thesis
      using A3 B6 by auto
  qed 
