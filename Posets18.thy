...
lemma ubd_double2b:
  "\<lbrakk>(a, b)\<in>R; a \<in>X; b \<in> X; trans R X\<rbrakk> \<Longrightarrow> x \<in> ubd R  X {b} \<Longrightarrow> x \<in> ubd R  X {a, b}"
  using ubd_double2 by fastforce

lemma ubd_double3:
  "\<lbrakk>(a, b)\<in>R; a \<in>X; b \<in> X; trans R X\<rbrakk> \<Longrightarrow> ubd R  X {a, b} = ubd R  X {b}"
  by (simp add: subset_antisym ubd_double1 ubd_double2)

lemma ubd_as_inter3:
  "ubd R X A = (\<Inter>a \<in> A. R``{a})\<inter>X"
  by(auto intro: set_eq_iff,simp add: ubdD1,simp add: ubd_mem_iff2,simp add: ubdI)
...
lemma binary_supD4:
  "\<lbrakk>trans R X; (s,s)\<in>R; (a, a) \<in>R; (b, b) \<in> R; a \<in> X; b \<in> X; c \<in> X; s \<in> X; is_sup R X {a, b} s\<rbrakk>  \<Longrightarrow> ((s, c) \<in> R \<longleftrightarrow> (a, c) \<in> R \<and> (b, c) \<in> R)"
...
lemma bsup_powrrel:
  "\<lbrakk>A \<in> Pow X; B \<in> Pow X\<rbrakk> \<Longrightarrow> is_sup (pwr X) (Pow X) {A, B} (A \<union> B)"
  by (metis Sup_insert bot.extremum ccpo_Sup_singleton insert_subset powrel5)
    ...
    apply(rule filterI1)
      ...
      proof(rule is_dirI1)
        ...
        proof-
          ...
          obtain B4:"aB \<inter> bB \<in> F"  using A0 B0 B1 A3 by (simp add: Posets18.A3) 
