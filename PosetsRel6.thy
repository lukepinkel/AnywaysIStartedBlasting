...
lemma Lim_to_Cl2: 
  ...
  shows Lim_to_Cl20:"\<And>x A. \<lbrakk>x \<in> X; A \<in> Pow X\<rbrakk> \<Longrightarrow> (A, x) \<in> (ClLim Lim X) \<longleftrightarrow> (\<exists>\<G> \<in> pfilters_on (pwr X) (Pow X). (\<G>, x) \<in> Lim \<and> A \<in> \<G>)"
proof-
