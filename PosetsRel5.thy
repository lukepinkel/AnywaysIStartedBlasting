...
abbreviation prtop_nh where
  "prtop_nh X N \<equiv> (\<forall>x. x \<in> X \<longrightarrow>  (N``{x}) \<in> pfilters_on (pwr X) (Pow X))"
...
proof-
  ...
  proof-
    ...
    proof
      ...
      proof(rule ccontr)
        assume A0:"\<not>(N``{x} \<subseteq> ?U)" then obtain V where B0:"V \<in> (N``{x})" and B1:"V \<notin> ?U"
          by auto
        then obtain "V \<in> Pow X"
          by (meson \<open>x \<in> X\<close> pfilters_on_iff prt sets_pfilter6 subsetD)
        then obtain "x \<in> (X-V)"
          by (metis B1 DiffI PowD \<open>x \<in> X\<close> empty_subsetI insert_subset lcroI1 pwr_mem_iff)
        then obtain "\<And>\<F>.  \<lbrakk>N``{x} \<subseteq> \<F>; \<F> \<in> pfilters_on (pwr X) (Pow X)\<rbrakk> \<Longrightarrow> (X-V) "
    then have "\<And>x. x \<in> X \<Longrightarrow> ((LimN N X)``{x}) \<in> pfilters_on (pwr X) (Pow X)"
    show "is_prtop X (LimN N X)"
  qed
qed
...
lemma prtp_ClLimD:
  ...
  shows prtp_ClLimD1:"(ClLim Lim X) = {(A, x). A \<in> Pow X \<and> x \<in> X \<and> (\<exists>\<F> \<in> pfilters_on (pwr X) (Pow X). A \<in> \<F> \<and> (\<F>, x) \<in> Lim)}" and
        prtp_ClLimD2:"(ClLim Lim X)``{{}}={}" and
        prtp_ClLimD3:"\<And>A. A \<in> Pow X \<Longrightarrow> A \<subseteq>(ClLim Lim X)``{A}" and
        prtp_ClLimD4:"\<And>A B. \<lbrakk>A \<in> Pow X; B \<in> Pow X\<rbrakk> \<Longrightarrow> (ClLim Lim X)``{A \<union> B}=(ClLim Lim X)``{A} \<union> (ClLim Lim X)``{B}" and
        prtp_ClLimD5:"\<And>A B. \<lbrakk>B \<in> Pow X; A \<subseteq> B\<rbrakk> \<Longrightarrow>  (ClLim Lim X)``{A} \<subseteq>  (ClLim Lim X)``{B}"
proof-
  obtain centered:"\<And>x. x \<in> X \<Longrightarrow> (lcro (pwr X) (Pow X) {x}, x) \<in> Lim"
  ...
  proof-
    ...
    qed
  qed
  show Q4:"\<And>A B. \<lbrakk>B \<in> Pow X; A \<subseteq> B\<rbrakk> \<Longrightarrow>  (ClLim Lim X)``{A} \<subseteq>  (ClLim Lim X)``{B}"
  proof-
    fix A B assume bmem:"B  \<in> Pow X" and aleb:"A \<subseteq> B"
    then obtain "A \<in> Pow X"
      by blast
    then show "(ClLim Lim X)``{A} \<subseteq>  (ClLim Lim X)``{B}"
      by (metis bmem aleb Q3 subset_Un_eq)
  qed
qed
...
proof-
  fix A x assume A2:"A\<in>(Pow X)" and A3:"x\<in>X"
...
proof-  
  ...
  proof
    ...
    have PF1:"Pow X \<noteq> \<F>"
      by (metis (no_types, lifting) CCl2 CollectD Diff_empty Pow_bottom Pow_top \<F>_def subsetD xmem)
    ...
    proof-
      ...
      then show "V \<inter> A \<noteq> {}" 
        using A0 A1 amem  xmem prtp_cl_mem_iff[of Cl X A x] R vix by blast
    qed
    have F1:"{A} # \<F> "
    ...
    proof-
      ...
      have E2:"\<And>V. \<lbrakk>V \<in> Pow X;  x \<notin> Cl `` {X - V}\<rbrakk> \<Longrightarrow> E \<inter> V \<noteq> {} "
        using E1 \<F>_def mesh_singleE by auto
      ...
      then show "(E, x) \<in> Cl" 
         using A0 A1 xmem prtp_cl_mem_iff[of Cl X E x] E0 by force
    qed
    then show ?L
  ...
  proof-
    ...
    proof-
      ...
      have B3:"(ClLim p Y)``{f`(?A)} \<subseteq>  (ClLim p Y)``{Y-V}"
        by (simp add: A4 prtp_ClLimD5 prtpy)
