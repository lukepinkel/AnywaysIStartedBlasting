...
lemma pfilter_on_lattice_sup01:
  "\<lbrakk>is_lattice X; is_pfilter X F; x \<in> X; y \<in> F\<rbrakk> \<Longrightarrow> Sup X {x, y} \<in> F "
...
lemma pfilters_upcl:
  "\<lbrakk>is_pfilter X F1; is_pfilter X F2\<rbrakk> \<Longrightarrow> is_ord_cl X (F1 \<inter> F2) (\<le>)"
...
definition sub_inf_semilattice::"'a::order set \<Rightarrow> 'a::order set \<Rightarrow> bool" where
  "sub_inf_semilattice X A\<equiv> (A \<subseteq> X) \<and> (\<forall>a b. a \<in> A \<and> b \<in> A \<longrightarrow> (\<exists>i \<in> A. is_inf X {a, b} i))"
